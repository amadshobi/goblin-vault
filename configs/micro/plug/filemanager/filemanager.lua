...
local function refresh_view()
	...
	local function file_icon(filename)
		...
		local basename = string.match(filename, "^([^.]*)$") or string.lower(filename)
		for name, icon in pairs(special_names) do
			if string.lower(filename) == name then
				return icon
			end
		end
	...
	for i = 1, #scanlist do
		...
		if scanlist[i].dirmsg ~= "" then
			-- Use folder icons for collapsed/expanded directory state
			local dir_icon = (scanlist[i].dirmsg == "+" and "📁" or "📂")
			display_content = dir_icon .. " " .. get_basename(scanlist[i].abspath)
		else
			-- Use file type icons
			local icon = file_icon(get_basename(scanlist[i].abspath))
			display_content = icon .. " " .. get_basename(scanlist[i].abspath)
		end
...
local function open_tree()
	-- Get active file's parent dir BEFORE creating the split
	local current_buf = micro.CurPane().Buf
	local start_path = os.Getwd()
	if current_buf ~= nil then
		local buf_path = current_buf.AbsPath
		if buf_path ~= nil and buf_path ~= "" then
			local parent_dir = filepath.Dir(buf_path)
...
function preMousePress(view, event)
	if view == tree_view then
		...
		-- Direct mapping: screen y = buffer y (no softwrap in tree view)
		-- y=2 → "..", y>=3 → files/dirs
		try_open_at_y(y)
