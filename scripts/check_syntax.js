...
function printArrowError(file, location, detail, snippet, fixHint = null) {
	...
	console.log(
		`        ${colors.gray}↳${colors.reset} ${colors.cyan}Location :${colors.reset} ${colors.yellow}${location}${colors.reset}`,
	);
	if (detail) {
		console.log(
			`        ${colors.gray}↳${colors.reset} ${colors.cyan}Details  :${colors.reset} ${colors.white}${detail}${colors.reset}`,
		);
	}
	if (snippet) {
		const cleanSnippet = snippet
			...
			.join(" | ");
		console.log(
			`        ${colors.gray}↳${colors.reset} ${colors.cyan}Code     :${colors.reset} ${colors.gray}${cleanSnippet}${colors.reset}`,
		);
	}
	if (fixHint) {
		console.log(
			`        ${colors.gray}↳${colors.reset} ${colors.cyan}Fix Hint :${colors.reset} ${colors.yellow}${fixHint}${colors.reset}`,
		);
	}
}
...
// ------------------------------------------------------------
console.log(
	`${colors.bold}${colors.blue}🔑 Executable Permissions (+x)${colors.reset}`,
);
...
execCheckTargets.forEach((relPath) => {
	...
	if (fs.existsSync(fullPath) && fs.statSync(fullPath).isFile()) {
		...
		if (!isExecutable) {
			...
			console.log(
				`   ${colors.green}✔${colors.reset} ${colors.gray}${relPath}${colors.reset}`,
			);
		}
	}
});
...
// ------------------------------------------------------------
console.log(
	`${colors.bold}${colors.cyan}🐚 Shell Scripts (Bash/Zsh)${colors.reset}`,
);
...
if (shellFiles.length > 0) {
	shellFiles.forEach((file) => {
		...
		if (fs.existsSync(fullPath)) {
			...
			if (res.status !== 0) {
				...
				console.log(
					`   ${colors.green}✔${colors.reset} ${colors.gray}${file}${colors.reset}`,
				);
			}
		}
	});
	...
	console.log(
		`   ${colors.gray}◦ No files to check in ${mode} mode.${colors.reset}`,
	);
}
console.log(`${colors.gray}─`.repeat(64) + `${colors.reset}`);
...
// ------------------------------------------------------------
console.log(
	`${colors.bold}${colors.magenta}🐹 Go Files & Modules (go vet)${colors.reset}`,
);
...
// ------------------------------------------------------------
console.log(
	`${colors.bold}${colors.yellow}📦 JavaScript (Node.js)${colors.reset}`,
);
...
if (jsFiles.length > 0) {
	jsFiles.forEach((file) => {
		const fullPath = path.join(ROOT_DIR, file);
		if (fs.existsSync(fullPath)) {
			const res = spawnSync("node", ["--check", fullPath], {
				encoding: "utf8",
			});
			if (res.status !== 0) {
				totalErrors++;
...
// ------------------------------------------------------------
console.log(
	`${colors.bold}${colors.blue}🔷 TypeScript & JSX (Bun)${colors.reset}`,
);
	...
	if (hasBun) {
		tsFiles.forEach((file) => {
			...
			if (fs.existsSync(fullPath)) {
				const res = spawnSync(
					"bun",
					["build", "--target=bun", "--no-save", fullPath],
					{ encoding: "utf8" },
				);
				if (res.status !== 0) {
					totalErrors++;
		...
		console.log(
			`   ${colors.yellow}⚠ Bun not found, skipping TypeScript checks.${colors.reset}`,
		);
	}
} else {
	console.log(
...
// ------------------------------------------------------------
// 6. Rust Files & Cargo Inspection (cargo clippy & fmt)
// ------------------------------------------------------------
console.log(
	`${colors.bold}${colors.yellow}🦀 Rust Files & Cargo Modules (cargo clippy)${colors.reset}`,
);
const rsFiles = getFilesToScan("rs");

if (rsFiles.length > 0) {
	const srcDir = path.join(TOOLS_DIR, "src");
	const cargoResults = new Map();

	if (fs.existsSync(srcDir)) {
		fs.readdirSync(srcDir).forEach((dir) => {
			const cargoPath = path.join(srcDir, dir, "Cargo.toml");
			if (fs.existsSync(cargoPath)) {
				const res = spawnSync(
					"cargo",
					["clippy", "--manifest-path", cargoPath, "--", "-D", "warnings"],
					{ encoding: "utf8" },
				);
				cargoResults.set(dir, res);
			}
		});
	}
	...
	rsFiles.forEach((file) => {
		...
		const relToTools = path.relative(path.join(TOOLS_DIR, "src"), fullPath);
		const modName = relToTools.split(path.sep)[0];
		const cargoRes = cargoResults.get(modName);

		if (cargoRes && cargoRes.status !== 0) {
			if (cargoRes.stderr.includes(path.basename(file))) {
				totalErrors++;
				const parsed = parseErrorDetails(file, cargoRes.stderr.trim(), "rust");
				errorSummaryList.push({
					file,
					location: parsed.location,
					detail: parsed.detail,
...
// ------------------------------------------------------------
// 7. Changelog Guardrail (Staged Mode Only)
// ------------------------------------------------------------
if (mode === "staged") {
	console.log(
		`${colors.bold}${colors.white}📜 Changelog Guardrail${colors.reset}`,
	);
	...
	if (stagedTools.length > 0) {
		...
		stagedTools.forEach((tool) => {
			...
			const hasClUpdate = stagedFiles.some(
				(f) => f === modularCl || f === "CHANGELOG.md",
			);
			if (!hasClUpdate) {
		...
		if (missingChangelogs.length > 0) {
			console.log(
				`   ${colors.yellow}⚠ WARNING:${colors.reset} Source modified for [${missingChangelogs.join(", ")}], missing staged changelog update.`,
			);
		} else {
			console.log(
				`   ${colors.green}✔ All tool changelogs staged.${colors.reset}`,
			);
		}
	} else {
...
if (totalErrors === 0) {
	console.log(
		`${colors.bold}${colors.green}  🎉 Hore BOSS! Semua syntax & permission valid! 🍻${colors.reset}\n`,
	);
	process.exit(0);
} else {
	console.log(
		`${colors.bold}${colors.red}  ❌ Waduh! Ditemukan ${totalErrors} kesalahan!${colors.reset}\n`,
	);
	console.log(
		`${colors.bold}${colors.red}🤨 NIH BOSS FILE YANG ERROR:${colors.reset}`,
	);
	errorSummaryList.forEach((err, idx) => {
		console.log(
			`   ${colors.bold}${colors.red}${idx + 1}.${colors.reset} ${colors.yellow}${err.location}${colors.reset}`,
		);
		console.log(`      ${colors.gray}Details :${colors.reset} ${err.detail}`);
		if (err.fixHint) {
			console.log(
				`      ${colors.gray}Fix Hint:${colors.reset} ${colors.cyan}${err.fixHint}${colors.reset}`,
			);
		}
	});
	console.log();
