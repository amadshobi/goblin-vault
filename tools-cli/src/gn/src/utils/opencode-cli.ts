 ...
 */
export function fetchAllSessionData(cutoffTs?: number, sessionFilter?: string | null) {
  ...
  return withOpenCodeDb((db) => {
    ...
    if (rootSessions.length === 0) {
      return { rootSessions: [], subagentsByParent: new Map(), toolsBySession: new Map(), diffsBySession: new Map() };
    }
    ...
    const rawDiffMap = new Map<string, Map<string, { additions: number; deletions: number }>>();
    ...
    for (const er of editRows) {
      ...
      const current = innerMap.get(er.filePath) ?? { additions: 0, deletions: 0 };
      if (er.tool === "edit") {
        current.additions += er.newString ? er.newString.split("\n").length : 0;
        current.deletions += er.oldString ? er.oldString.split("\n").length : 0;
    ...
    for (const [sessId, fileMap] of rawDiffMap.entries()) {
      ...
      for (const [filePath, diff] of fileMap.entries()) {
        stats.push({ filePath, additions: diff.additions, deletions: diff.deletions });
      }
...
export function fetchRootSessions(cutoffTs?: number, sessionFilter?: string | null): OpenCodeCliSessionRow[] {
  return fetchAllSessionData(cutoffTs, sessionFilter).rootSessions;
}

export function fetchSubagentSessions(parentIds: string[]): OpenCodeCliSessionRow[] {
...
export function fetchSessionTools(sessionId: string): OpenCodeCliToolUsageRow[] {
