...
function printDoctorHelp(): void {
	const lines = [
		...
		"  8. Cache dir ~/.config/gn/cache     directory writable",
...
function checkSecretVault(): DoctorCheckResult {
	...
	try {
		...
		return {
			...
			hint:
				count === 0
					? "Tambah file .json berisi token/cookie di subfolder provider."
					: undefined,
		};
	} catch (err) {
		return {
...
async function isServiceActive(service: string): Promise<boolean> {
	try {
		const proc = Bun.spawn(["systemctl", "--user", "is-active", service], {
			stdout: "pipe",
			stderr: "pipe",
		});
