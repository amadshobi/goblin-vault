...
export async function handleConfigCommand(argv: string[]): Promise<number> {
  ...
  if (!config) {
    stderr.write(`❌ Gagal membaca atau mem-parsing file konfigurasi di: ${configPath}\n`);
    return 1;
  }
  ...
  if (action === "get") {
        ...
        for (const [k, v] of Object.entries(providers)) {
          if (typeof v === "string") {
            ...
            if ("api_key" in maskedProviders[k]) {
              maskedProviders[k].api_key = maskApiKey(maskedProviders[k].api_key);
            }
          }
        }
    ...
    if (target === "agent") {
      ...
      if (agents.length === 0) {
        ...
        const headers = ["ID", "Name", "Model", "Permissions Count", "Tools Count"];
      ...
      console.log(`    Keep Percent    : ${comp.keep_percent ? comp.keep_percent + "%" : "-"}`);
      
      const features = config.features || {};
      console.log(`\n  Features:`);
      for (const [k, v] of Object.entries(features)) {
        if (k !== "providers") {
          console.log(`    ${k.padEnd(16)}: ${typeof v === "boolean" ? formatBoolStatus(v) : v}`);
        }
      }
    } else if (target === "providers") {
      ...
      if (models.length === 0) {
        ...
        const headers = ["ID", "Provider", "Input / M", "Output / M", "Context Window"];
      ...
      console.log(`${ANSI_YELLOW}󰀦 Target get tidak valid. Gunakan: agent | mcp | settings | providers | models${ANSI_RESET}`);
    }
    console.log();
    return 0;
  }
  ...
  if (action === "set") {
    ...
    try {
      ...
      stderr.write(`❌ Gagal membuat backup file konfigurasi di: ${backupPath}\n`);
      return 1;
    }
    ...
    if (success) {
      ...
      console.log(`${ANSI_GRAY}   Backup dibuat di: ${backupPath}${ANSI_RESET}\n`);
      return 0;
    } else {
      stderr.write(`❌ Gagal menulis pembaruan konfigurasi ke file: ${configPath}\n`);
      return 1;
    }
  }
